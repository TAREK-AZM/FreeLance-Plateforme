...
export default function FreelancerDetails() {
    const [SartConversation, setStartConversation] = useState(false);
    const { id } = useParams()
  ...
  const scrollAreaRef = useRef(null);
    ...
    const handleFavorite = async () => {
        ...
        try {
            const token = localStorage.getItem("token"); // Retrieve token
            const response = await axios.post(`${BASE_URL}/freelancers/${id}/favorite`, {
    ...
    const onSubmit = async ({ name, rating, comment, time }) => {
        // add feedback logic here
        try {
            const token = localStorage.getItem("token"); // Retrieve token
            const response = await axios.post(`${BASE_URL}/freelancers/${id}/feedback`, {
                name,
                rating,
                comment,
                time,
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid lg:grid-cols-3 gap-8 mt-12">
                    ...
                    <div className="flex flex-col lg:flex-row gap-8 items-start ">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex gap-4">
                                ...
                                <Link onClick={() => setStartConversation(true)}>
                                    <Button className="rounded-full bg-[#12AE65] hover:bg-[#0d8d52]">Send Message</Button>
                                </Link>
                    ...
                    <Card className="lg:col-span-2 p-6">
                        ...
                        <div className="mt-8 space-y-4">
                            ...
                            <Card className="mt-8 p-6">
                                ...
                                {/* Recent Reviews */}
                                <ScrollArea className="h-[300px] flex-grow p-4 border border-1 border-gray-300 rounded-lg" ref={scrollAreaRef}>
                                <div className="mt-8 space-y-4 ">
                ...
                {/* conversation modal */}
                { SartConversation ? 
                <ScrollArea className="flex-grow p-4 border border-1 border-gray-300 rounded-lg">
                    <ConversationModal />
